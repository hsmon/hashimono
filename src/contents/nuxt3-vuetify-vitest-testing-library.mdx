...
slug: nuxt3-vuetify3-vitest-vue-testing-library
author: hsmon
...
Vitest で Vuetify を利用するためには、`vite-plugin-vuetify`も必要になります。
...
テストコードを実装していきますが、Testing Library に Vuetify をマウントする必要があるため、まずは`renderWithVuetify`というカスタムラッパーの関数を用意します。
...
重要になるのは`render`関数のオプションで用意している`global.plugins`キーで、ここに Vuetify を読み込ませる必要があります。
...
Vue2 の場合、以下 testing-library が用意していたコードサンプルを元に実装していましたが、Vue3 の場合は上記のように`global.plugins`キーを利用する必要があります。  
自分が探す限り、どこにもそのようなサンプルコードがなかったので結構時間を取られてしまいました。。

<OG url="https://github.com/testing-library/vue-testing-library/blob/main/src/__tests__/vuetify.js" />
...
<OG url="https://github.com/hsmon/nuxt3-vuetify-vitest-testing-library" />
